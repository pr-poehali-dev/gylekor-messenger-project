...
interface AuthScreenProps {
  onAuth: (user: { id: string; phone: string; username: string; name: string }) => void;
}
...
export default function AuthScreen({ onAuth }: AuthScreenProps) {
  ...
  const handleProfile = async () => {
    ...
    setError("");
    try {
      const user = await registerUser(phone, username.replace("@", ""), name);
      onAuth(user);
    } catch (e: unknown) {
        ...
        {step === "phone" && (
          <div className="animate-fade-in">
            <div className="glass-strong rounded-2xl p-6">
              ...
              />
              <button onClick={handlePhone} disabled={phone.length < 10 || loading}
                ...
                {loading ? <span className="flex items-center justify-center gap-2"><div className="w-4 h-4 rounded-full border-2 border-white/40 border-t-white animate-spin" />Отправляем...</span> : "Получить код"}
              </button>
            </div>
          </div>
        )}
        ...
        {step === "code" && (
          <div className="animate-fade-in">
            <div className="glass-strong rounded-2xl p-6">
              ...
              />
              <button onClick={handleCode} disabled={code.length < 4 || loading}
                ...
                style={{ background: 'linear-gradient(135deg, #7c3aed, #ec4899)' }}>
                {loading ? <span className="flex items-center justify-center gap-2"><div className="w-4 h-4 rounded-full border-2 border-white/40 border-t-white animate-spin" />Проверяем...</span> : "Подтвердить"}
              </button>
        ...
        {step === "profile" && (
          <div className="animate-fade-in">
            <div className="glass-strong rounded-2xl p-6">
              ...
              <button onClick={handleProfile} disabled={!name.trim() || !username.trim() || loading}
                className="w-full py-3.5 rounded-xl font-semibold text-white transition-all duration-200 disabled:opacity-40 hover:scale-[1.02] active:scale-[0.98]"
                style={{ background: 'linear-gradient(135deg, #7c3aed, #ec4899)' }}>
                {loading ? <span className="flex items-center justify-center gap-2"><div className="w-4 h-4 rounded-full border-2 border-white/40 border-t-white animate-spin" />Создаём...</span> : "Войти в Гылекор 🚀"}
              </button>
            </div>
          </div>
        )}

        <p className="text-center text-muted-foreground text-xs mt-6">Регистрируясь, вы соглашаетесь с условиями использования</p>
      </div>
    </div>
  );
}
